...
pub mod highspeed {
    ...
    /// with a [`HSConfig`][`crate::HSConfig`] instruction has a zero-width
    ...
    /// the `DAC+` value of the last [`SetDAC`][`crate::SetDAC`] instruction. If
    /// `polarity` is `false` (low) then ArC2 will revert to `DAC-`. If instead
